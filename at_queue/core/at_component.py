...
from at_queue.core.session import MSGAwaitSession
from at_queue.utils.schema_inspector import create_args_model
from at_queue.utils.schema_inspector import method_to_json_schema
from at_queue.utils.schema_inspector import return_type_to_json_schema
...
class Parameter:
    ...
    @property
    def __dict__(self):
        return {"schema": self.schema, "strict_validation": self.strict_validation, "description": self.description}
...
class BaseComponentMethod:
    ...
    def __dict__(self):
        return {
            ...
            "inputs": {input_name: input.__dict__ for input_name, input in self.inputs.items()},
...
class ATComponentMethod(BaseComponentMethod):
    ...
    def __init__(self, orig: Callable) -> None:
        ...
        if orig.__name__ == "<lambda>":
            raise TypeError(
                "Can't create a component method for anonymous function")

        name = orig.__name__
        inputs = self.get_inputs()
        output = self.get_output()
        super(ATComponentMethod, self).__init__(
            name=name, inputs=inputs, output=output)

    def _check_owner(self):
        if not self.owner:
            raise ValueError(
                f'Missing ATComponent instance to call "{self.name}"')
        return True
    ...
    def get_inputs(self) -> Dict[str, Input]:
        ...
        for name, schema in args_schema["properties"].items():
            result[name] = Input(
                name=name, schema=schema, required=name in args_schema.get("required", []), method=self
            )
        return result
    ...
    def get_output(self) -> Output:
        return Output(schema=self.return_schema, method=self)
    ...
    def __call__(self, *args, **kwargs):
        ...
        if "auth_token" not in kwargs:
            raise exceptions.NotAuthorizedMethodException(
                ...
                processed_message_id=self.metadata.get(
...
class BaseComponent:
    ...
    def __dict__(self):
        return {
            "name": self.name,
            "description": self.description,
            "methods": {method_name: method.__dict__ for method_name, method in self.methods.items()},
        }
...
class ATComponent(BaseComponent, metaclass=ATComponentMetaClass):
    ...
    _passed_configs: Dict[str, ATComponentConfig]
    ...
    def __init__(self, connection_parameters: ConnectionParameters, *args, **kwargs):
        ...
        self.register_session = MSGAwaitSession(
            self.name,
            connection_parameters,
            uuid3(NAMESPACE_OID, "at_registry"),
            auto_ack=False,
            warn_on_reciever_differs=False,
        )
    ...
    def _check_initialized(self):
        if not self.initialized:
            raise exceptions.ComponentNotInitializedException(
                f"Component {self.name} is not initialized", self.session, self
            )

    async def register(self):
        self._check_initialized()
        message = {"type": "register", "component": self.__dict__}
        self.register_listen_task = asyncio.get_event_loop(
        ).create_task(self.register_session.listen())
    ...
    async def _register_callback(
        ...
        **kwargs,
        ...
        if not self.registered and (reciever == self.name):
            logger.info(
                "Register component callback received message with id " f"{message_id} about registering: {message}"
            )
            if sender != "registry":
                logger.warning(
                    f'Register component callback received message not from registry, but from "{sender}"')
            if str(answer_to) == str(self.register_msg_id):
                if errors is not None:
                    ...
                    raise exceptions.RegisterException(
                        msg, self.session, self, message_id, message)
                await msg.ack()

                session_id = message.get("session_id")
                self.session = MSGAwaitSession(
                    self.name, self.register_session.connection_parameters, uuid=session_id)
    ...
    async def _log_process_message(self, *args, message: dict, sender: str, reciever: str, message_id: str, **kwargs):
        logger.info(
            f'Start for "{self.name}" processing message {message_id}: {message} from "{sender}" with arguments: '
            f'{args}, {kwargs}'
        )
        if reciever != self.name:
            logger.warning(
                f'Component "{self.name}" received message that is not for "{self.name}", but for "{reciever}"'
            )

        await self._process_message(
            self, message=message, sender=sender, reciever=reciever, message_id=message_id, *args, **kwargs
        )

        logger.info(
            f'Finish for "{self.name}" processing message {message_id}')

    async def _process_message(
        self, *args, message: dict, sender: str, reciever: str, message_id: str, msg: aio_pika.IncomingMessage, **kwargs
    ):
        message_type = message.get("type")
        try:
            if message_type == "ping":
                await self.session.send(
                    reciever=sender, message={"result": "pong"}, answer_to=message_id, **msg.headers
                )
            if message_type == "configurate":
                await self._configurate(
                    *args, message=message, sender=sender, message_id=message_id, reciever=reciever, msg=msg, **kwargs
                )
            if message_type == "check_configured":
                configurated = await self._is_configured(
                    *args, message=message, sender=sender, message_id=message_id, reciever=reciever, msg=msg, **kwargs
                )
                await self.session.send(
                    reciever=sender, message={"result": configurated}, answer_to=message_id, **msg.headers
                )
            if message_type == "exec_method":
                await self._exec_method(
                    *args, message=message, sender=sender, message_id=message_id, reciever=reciever, msg=msg, **kwargs
                )
    ...
    async def _is_configured(
        ...
        **kwargs,
        ...
        return await self.check_configured(
            *args,
            message=message,
            sender=sender,
    ...
    async def _configurate(
        self, *args, message: dict, sender: str, message_id: Union[str, UUID], msg: aio_pika.IncomingMessage, **kwargs
    ):

        config_data = message.get("config")
        if config_data is None:
            msg = f'Component "{self.name}" received configurate message with id "{message_id}" from "{sender}" '
            msg += 'but configuration is not specified'
            e = exceptions.ProcessMessageException(
                msg, self.session, self, message)
        ...
        config = await load_component_config(config_data, context=Context(component=self, auth_token=auth_token))
        if cache_key:
            self._passed_configs[cache_key] = config
        try:
            result = await self.perform_configurate(config, auth_token=auth_token)
            await self.session.send(
                reciever=sender, message={"result": result}, answer_to=message_id, await_answer=False
            )
        except Exception as e:
            logger.error(traceback.format_exc())
            await self.session.send(
                reciever=sender, message={"errors": [str(e)]}, answer_to=message_id, await_answer=False
            )
    ...
    async def _exec_method(
        ...
        if method_name is None:
            msg = f'Component "{self.name}" received execute method message with id "{message_id}" from "{sender}" '
            msg += 'but method name is not specified'
            e = exceptions.ProcessMessageException(
                msg, self.session, self, message)
            logger.error(e.__dict__)
            self.session.send(
        ...
        if method is None:
            msg = f'Component "{self.name}" received execute method message with id "{message_id}" from "{sender}" '
            msg += f'but got unknown method name "{method_name}". Avalible method names are: {[
                m for m in self.methods]}'
            e = exceptions.ExecMethodException(
                msg, self.session, self, message_id, message)
            logger.error(e.__dict__)
            await self.session.send(
                reciever=sender, message={"errors": [e.__dict__]}, answer_to=message_id, await_answer=False
        ...
        try:
            auth_token = msg.headers.get("auth_token") if isinstance(
            ...
            for field_name in validated_data.model_dump():
                exec_kwargs[field_name] = getattr(validated_data, field_name)

            for f in method_args:
        ...
        try:
            result = method.execute(
                processed_message=message, processed_message_id=message_id, exec_arguments=exec_kwargs
            )
    ...
    async def exec_external_method(
        self, reciever: str, methode_name: str, method_args: dict, auth_token: str = None
    ) -> dict:
        ...
        exec_result = await self.session.send_await(
            reciever=reciever, message={"type": "exec_method", "method": methode_name, "args": method_args}, **headers
        )
...
{json.dumps(errors, indent=4, ensure_ascii=False)}"""
            logger.error(msg)
            raise exceptions.ExternalMethodException(
                msg, self.session, self, exec_result.get(
                    "message_id"), exec_result
            )
        if exec_result.get("type") != "method_result":
            msg_type = exec_result.get("type")
            logger.warning(
                f'Recieved unexpected message type "{msg_type}". Expected: "method_result"')
