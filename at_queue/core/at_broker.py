...
class ATBrokerInstance:
    ...
    def __init__(
        ...
        self.session = CommunicationReversedSession(
            self.component.name, self.connection_parameters, uuid=session_id)
    ...
    async def _log_process_message(self, *args, message: dict, sender: str, reciever: str, message_id: str, **kwargs):
        logger.info(
            f'Start for "{self.component.name}" broker processing sent message {message_id}: '
            f'{message} from "{sender}" with arguments: {args}, {kwargs}'
        )
        if sender != self.component.name:
            logger.warning(
                f'Component "{self.component.name}" sended message that is not from "{self.component.name}", '
                f'but from "{reciever}"'
            )

        await self._process_message(
            message=message, sender=sender, reciever=reciever, message_id=message_id, *args, **kwargs
        )

        logger.info(
            f'Finish for "{self.component.name}" broker processing message {message_id}')
        ...
        if reciever == "registry":
            if message.get("type") == "check_registered":
                ...
                await self.session.send(
                    sender, {"result": component in self.registry._registry}, answer_to=message_id, sender="registry"
                )
            ...
            if reciever_broker:
                await reciever_broker.session.send(
                    reciever, message, answer_to=answer_to, message_id=message_id, sender=sender, **msg.headers
                )
            else:
                await self.session.send(
                    sender,
                    {"errors": [f'Component "{reciever}" is not registered yet']},
                    answer_to=message_id,
                    sender="registry",
                )
